...
async function upsertLedgerRecord(record: LedgerRecord): Promise<void> {
  if (!phase3FeatureFlags.enableDbLedger) {
    localLedgerAdapter.upsert(record);
    return;
  }
  ...
  if (!response.ok) {
    throw new Error(`ledger_upsert_failed_${response.status}`);
  }
}
...
export default function StudioWorkspace() {
  ...
  const saveArtifact = async () => {
    ...
    try {
      ...
      setSaveError(error instanceof Error ? error.message : "ledger_write_failed");
    }
  };
  ...
  return (
    <section className="space-y-4">
      ...
      <button className="rounded bg-slate-900 px-3 py-2 text-sm font-medium text-white disabled:opacity-50" type="button" onClick={() => void saveArtifact()} data-tour="artifact-save" disabled={!phase3FeatureFlags.enableRuntime && !phase3FeatureFlags.enableLedger}>
        Save Artifact
      </button>
